  ...
  class DefaultQueryGeneratorService
    ...
    def call(query_key: DEFAULT_QUERY)
      params =
        case query_key
        when DEFAULT_QUERY
          self.class.all_open_query
        when :latest_activity
          self.class.latest_activity_query
        when :recently_created
          self.class.recently_created_query
        when :overdue
          self.class.overdue_query
        when :summary
          return if project.blank?

          self.class.summary_query
        when :created_by_me
          return unless User.current.logged?

          self.class.created_by_me_query
        when :assigned_to_me
          return unless User.current.logged?

          self.class.assigned_to_me_query
        when :shared_with_users
          return unless User.current.logged?

          self.class.shared_with_users_query
        when :shared_with_me
          return unless User.current.logged?

          self.class.shared_with_me_query
        end
