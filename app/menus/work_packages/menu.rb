  ...
  class Menu < Submenu
    ...
    def query_path(query_params)
      if !EnterpriseToken.allows_to?(:work_package_sharing) &&
        (query_params[:name] == :shared_with_users || query_params[:name] == :shared_with_me)
        return share_upsale_work_packages_path
      end

      if project.present?
        return report_project_work_packages_path(project) if query_params[:name] == :summary
